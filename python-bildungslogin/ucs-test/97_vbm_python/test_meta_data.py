...
def test_number_of_available_licenses(licence_handler, meta_data_handler, random_meta_data, random_n_random_licenses):
    total_amount_of_licenses_for_product = 0
    for l in random_n_random_licenses:
        # comment: we do not have to create the meta-data for this
        l.product_id = random_meta_data.product_id
        licence_handler.create(l)
        total_amount_of_licenses_for_product += l.license_quantity
    assert meta_data_handler.get_number_of_available_assignments(random_meta_data) == total_amount_of_licenses_for_product


def test_number_of_provisioned_and_assigned_licenses(licence_handler, meta_data_handler, assignment_handler, random_meta_data, random_license):
    ...
    with utu.UCSTestSchool() as schoolenv:
        ...
        assignment_handler.assign_users_to_license(usernames=users,
         licenses_code=random_license.license_code)
        num_assigned = meta_data_handler.get_number_of_provisioned_and_assigned_assignments(random_meta_data)
        assert num_assigned == num_students + num_teachers
        for user_name in users[:2]:
            assignment_handler.change_license_status(username=user_name,
             license_code=random_license.license_code, status=Status.PROVISIONED)
...
def test_number_of_expired_licenses(licence_handler, meta_data_handler, random_meta_data, random_n_expired_licenses):
    total_amount_of_licenses_for_product = 0
    # a assignment is not expired if end_date + duration < today
    for l in random_n_expired_licenses:
        l.product_id = random_meta_data.product_id
        licence_handler.create(l)
        total_amount_of_licenses_for_product += licence_handler.get_number_of_expired_assignments(l)
    assert meta_data_handler.get_number_of_expired_assignments(random_meta_data) == total_amount_of_licenses_for_product


def test_total_number_of_licenses(licence_handler, meta_data_handler, random_meta_data, random_n_random_licenses, random_n_expired_licenses):
    total_amount_of_licenses_for_product = 0
    ...
    for l in itertools.chain(*all_licenses):
        l.product_id = random_meta_data.product_id
        licence_handler.create(l)
        total_amount_of_licenses_for_product += licence_handler.get_number_of_assignments(l)
    assert meta_data_handler.get_number_of_assignments(random_meta_data) == total_amount_of_licenses_for_product
