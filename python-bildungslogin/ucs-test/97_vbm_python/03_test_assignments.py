...
def test_assign_user_to_license(assignment_handler, licence_handler, license, user_type):
...
def test_assign_users_to_licenses(assignment_handler, licence_handler, license):
    # 00_vbm_test_assignments
    with utu.UCSTestSchool() as schoolenv:
        ou, _ = schoolenv.create_ou()
        license.license_school = ou
...
def test_get_assignments_for_product_id_for_user(licence_handler, assignment_handler, license):
    with utu.UCSTestSchool() as schoolenv:
        ou, _ = schoolenv.create_ou()
        license.license_school = ou
        licence_handler.create(license)
...
@pytest.mark.parametrize("user_type", ["student", "teacher"])
def test_positive_change_license_status(licence_handler, assignment_handler, user_type, license):
...
@pytest.mark.parametrize("user_type", ["student", "teacher"])
def test_negative_change_license_status(licence_handler, assignment_handler, user_type, license):
