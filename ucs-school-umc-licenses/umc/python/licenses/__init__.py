...
class Instance(SchoolBaseModule):
	...
	def query(self, request):
		...
		result = [
			l for l in licenses
			if (
				any(pattern.match(l[field]) for field in fields)
				and (True if not only_allocatable_licenses else l['countAllocatable'] > 0)
				and (True if publisher == '__all__' else l['publisher'] == publisher)
				and (True if license_type == '__all__' else l['licenseType'] == license_type)
				and (True if not time_from else l['importDate'] >= time_from)
				and (True if not time_to else l['importDate'] <= time_to)
			)
		]
	...
	def get(self, licenseId):
		...
		if licenseId == 0:
			users = [
				...
				{ 'dn': 'dn2', 'username': 'bobby', 'status': 'allocated', 'allocationDate': '2021-03-13' },
				{ 'dn': 'dn3', 'username': 'daniel', 'status': 'provisioned', 'allocationDate': '2021-02-08' },
			]
		else:
			users = [
				{ 'dn': 'dn4', 'username': 'xena', 'status': 'allocated', 'allocationDate': '2021-02-13' },
				{ 'dn': 'dn5', 'username': 'arnold', 'status': 'allocated', 'allocationDate': '2021-03-13' },
				{ 'dn': 'dn6', 'username': 'danny', 'status': 'provisioned', 'allocationDate': '2021-02-08' },
			]
		result = {
			'licenseId': 0,
			'productId': 'xxx-x-xxxxx-xxx-x',
	...
	def license_types(self):
		...
		result = [{
			'id': 'Volumenlizenz',
			'label': _('Volumenlizenz')
		}, {
			'id': 'Einzellizenz',
			'label': _('Einzellizenz')
		}]
	...
	def set_ignore(self, license_id, ignore):
		MODULE.info('licenses.set_ignore: license_id: %s ignore: %s' % (license_id, ignore,))
		return True
	...
	def users_query(self, request):
		...
		MODULE.info('licenses.query: options: %s' % str(request.options))
		result = [
			{ 'userId': 0, 'username': 'bmusterm', 'firstname': 'Bernd', 'lastname': 'Mustermann', 'class': '5C', 'workgroup': 'Singen' },
			{ 'userId': 1, 'username': 'amusterf', 'firstname': 'Anna', 'lastname': 'Musterfrau', 'class': '4A', 'workgroup': 'Fußball' },
			{ 'userId': 2, 'username': 'imusterm', 'firstname': 'Immanuel', 'lastname': 'Mustermann', 'class': '5B', 'workgroup': 'Fußball' },
			{ 'userId': 3, 'username': 'lmusterf', 'firstname': 'Linda', 'lastname': 'Musterfrau', 'class': '5C', 'workgroup': 'Singen' },
		]
